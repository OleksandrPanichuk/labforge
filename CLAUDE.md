...
- Мова коду/комітів/ідентифікаторів — англійська; промпти агентів і тексти для юзера — українська.
- **Clean Architecture в `apps/core`** (деталі: скіл `clean-architecture` + README у кожному шарі): `domain` (сутності, порти) ← `application` (use cases, стейт-машина) ← `infrastructure` (адаптери) / `presentation` (контролери, SSE). Залежності тільки всередину; `domain` і `application` не імпортують NestJS-інфраструктуру, Prisma, BullMQ, dockerode чи Agent SDK напряму — тільки через порти.
- **Коментарі в коді:** пиши коментар ЛИШЕ коли інформацію неможливо вивести з самого коду (зовнішнє обмеження, неочевидний інваріант, причина неочевидного рішення). Перед тим як додати — подумай двічі: (1) чи можна виразити це кодом (назвою, типом, структурою)? якщо так — зроби так, без коментаря; (2) чи справді читач без нього помилиться? якщо ні — не додавай. Заборонено: коментарі-переказ наступного рядка, "what" замість "why", історія змін, коментарі-виправдання для рев'ювера.
