...
Читання файлів іде через порт `JobFiles`; `jobFilesAt(jobDir)` не дає вийти за межі
job-директорії. Без `jobDir`/`files` документ із зображеннями падає з явною помилкою.
